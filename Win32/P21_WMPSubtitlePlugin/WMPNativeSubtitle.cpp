...
CWMPNativeSubtitle::CWMPNativeSubtitle():
    ...
    m_bLogSetting(FALSE),
    //m_bTempSAMI(TRUE)
    m_bTempSAMI(FALSE)    // until all methods implemented and property box is used
{
  ...
  // also consider whether location: kwszPrefsRegKey is readable/writeable
  ...
  if (ERROR_SUCCESS == lResult)
  {
    ...
    lResult = key.QueryValue(kwszPrefsSMILocation, &dwType, &dwValue, &uLength);

    if (ERROR_SUCCESS == lResult)
    {
      m_bTempSAMI = (BOOL)(dwValue & 0x0001);
    }
  }
}
...
BOOL CWMPNativeSubtitle::EnableCaptionFromSubtitle() {
  ...
  SubToSAMIConverter subToSAMIConverter(subInputName, (BOOL)(m_bLogSetting & 0x0001), (BOOL)(m_bTempSAMI & 0x0001));
  if (subToSAMIConverter.convert_to_sami()) {
    if (m_bTempSAMI) {
      BSTR extSmiFileName = ::SysAllocString(subToSAMIConverter.GetSMIFileName());
      BSTR mediaFileName;
      ...
      if (FAILMSG(hr)) {
        // MessageBox(NULL, TEXT("Interface not found"), TEXT("IWMPClosedCaption"), MB_OK);
        return FALSE;
      }
      hr = m_spCore->close();
      if (FAILMSG(hr))
      ...
      if (spWMPClosedCaption)
      {
        ...
        if (FAILMSG(hr))
          // MessageBox(NULL, TEXT("put smi failed"), TEXT("IWMPClosedCaption"), MB_OK);
          return FALSE;
      }
      hr = m_spCore->put_URL(mediaFileName);
      if (FAILMSG(hr))
        return FALSE;
...
BOOL CWMPNativeSubtitle::CaptionAlreadyAvailable() {
  ...
  hr = m_spCore->get_closedCaption(&spWMPClosedCaption);
  // Interface exists even if caption does not exist, so a failure doe not occur really
  if (FAILMSG(hr)) {
    // MessageBox(NULL, TEXT("Interface not found"), TEXT("IWMPClosedCaption"), MB_OK);
    return true;
  }
