...
// Constructor
//
SubToSAMIConverter::SubToSAMIConverter(LPTSTR infileName, BOOL bLogStatus, BOOL bSAMIInTemp) :
errorOccurred(FALSE),
firstReadData(TRUE),
encodingFormat(UNKNOWN_TEXT_TYPE),
m_bLogEnabled(bLogStatus),
m_bSAMIInTemp(bSAMIInTemp)
{
  inSubFileName = infileName;

  if (m_bSAMIInTemp)
  {
    // Get temp sami file path
    ...
    _tcscat_s(lpLogFilePathBuffer, MAX_PATH, TEXT("\\wmp_native_sub.smi"));
  }
  else {
  ...
  // This is the technique if we want log file to be created in same location of the media file
  ...
  if (m_bLogEnabled) {
    ...
    hLogFile = CreateFile(lpLogFilePathBuffer,                // name of the write
      ...
      0,                      // do not share
      NULL,                   // default security
      CREATE_ALWAYS,          // Create always, erase previous
      FILE_ATTRIBUTE_NORMAL,  // normal file
      NULL);                  // no attr. template
...
// Read entire input file here
  ...
  if (dwBytesRead > 0 && dwBytesRead <= READBUFFERSIZE-1)
  {
    ...
    if (ReadBufferW) {
      ...
      _tcscat_s(&samiBuffer[len], SMIBUFFERSIZE-len, ReadBufferW);
      if (ReadBufferW) {
        delete ReadBufferW;
        ReadBufferW = NULL;
      }
...
// We shouldn't allocate data here as caller pointer will not point to the allocated memory location
...
LINETYPE get_line_type(LPTSTR line, SubToSAMIConverter* pSamiConverter) {
  ...
  // Considering highest length of sequence is MAX_SEQ_LENGTH
...
DWORDLONG calculate_timestamp_ms(LPTSTR str, BOOL isEndingTime, SubToSAMIConverter* pSamiConverter) {
  ...
  // _tcstok_s modifies original string and returns pointer to the original as token
  LPTSTR time_str = _tcstok_s(str, delim, &next_token);
  if (time_str == NULL)
    return 0;

  if (isEndingTime) {
    time_str = _tcstok_s(NULL, delim, &next_token);
    if (time_str == NULL)
      return 0;
  ...
  // get hour
  LPTSTR token = _tcstok_s(time_str, delim_time, &next_token);
  if (token == NULL)
    return 0;
  int hour = _wtoi(token);

  // get minute
  token = _tcstok_s(NULL, delim_time, &next_token);
  if (token == NULL)
    return 0;
